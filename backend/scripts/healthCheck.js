...
const timeout = (ms, label) => new Promise((_, reject) => setTimeout(() => reject(new Error(`Timeout after ${ms}ms (${label})`)), ms));
...
async function checkMongo() {
  if (!process.env.MONGODB_URI) return { ok: false, error: 'MONGODB_URI missing' };
  const conn = await mongoose.createConnection(process.env.MONGODB_URI, { serverSelectionTimeoutMS: 3000 }).asPromise();
...
async function checkCloudinary() {
  if (!process.env.CLOUDINARY_CLOUD_NAME) return { ok: false, error: 'Cloudinary env incomplete' };
  try {
...
async function checkRedis() {
  if (!redisClient) return { ok: true, skipped: true, reason: 'Redis disabled or not configured' };
  try {
...
async function checkEmail() {
  ...
  if (!EMAIL_HOST || !EMAIL_PORT || !EMAIL_USER || !EMAIL_PASSWORD) {
    return { ok: true, skipped: true, reason: 'Email creds not fully provided' };
  }
  try {
...
(async () => {
  ...
  const criticalOk = summary.environment.ok && summary.mongo.ok && summary.cloudinary.ok;
  ...
  if (asJson) {
    ...
    console.log('================================');
    console.log('Env vars:      ', summary.environment.ok ? 'OK' : `Missing: ${summary.environment.missing.join(',')}`);
    console.log('MongoDB:       ', summary.mongo.ok ? 'OK' : `FAIL (${summary.mongo.error || summary.mongo.state})`);
    console.log('Cloudinary:    ', summary.cloudinary.ok ? 'OK' : `FAIL (${summary.cloudinary.error || summary.cloudinary.rawStatus})`);
    console.log('Redis:         ', summary.redis.skipped ? 'SKIPPED' : (summary.redis.ok ? 'OK' : `FAIL (${summary.redis.error})`));
    console.log('Email:         ', summary.email.skipped ? 'SKIPPED' : (summary.email.ok ? 'OK' : `FAIL (${summary.email.error})`));
    console.log('Timings (ms):  ', summary.timingsMs);
    console.log('================================');
    console.log(criticalOk ? 'ALL CRITICAL CHECKS PASSED' : 'ONE OR MORE CRITICAL CHECKS FAILED');
  }
