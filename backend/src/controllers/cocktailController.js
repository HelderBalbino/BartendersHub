...
export const getCocktail = async (req, res) => {
  try {
    const cocktail = await Cocktail.findById(req.params.id)
      .populate('createdBy', 'name username avatar isVerified speciality location')
...
export const updateCocktail = async (req, res) => {
  try {
    ...
    if (cocktail.createdBy.toString() !== req.user.id && !req.user.isAdmin) {
...
export const addComment = async (req, res) => {
  try {
    ...
    await cocktail.populate('comments.user', 'name username avatar isVerified');
