...
const CountrySelect = ({ value, onChange, onBlur, error, touched, required }) => {
  ...
  onBlur={() => {
      ...
      <div className='pointer-events-none absolute top-1/2 right-3 -translate-y-1/2 text-yellow-400'>⌄</div>
      {open && (
        <ul
          id='country-listbox'
          role='listbox'
          ...
          {filtered.map((c, idx) => (
            <li
              ...
              className={`flex items-center gap-2 px-3 py-2 cursor-pointer text-sm ${
                idx === highlight ? 'bg-yellow-400/20' : 'hover:bg-yellow-400/10'
              } ${value === c.code ? 'text-yellow-300' : 'text-gray-200'}`}
