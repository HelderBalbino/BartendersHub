 ...
 */
const RequireVerification = ({ children, reason = 'perform this action', inline = false, intent = 'generic', onRedirectIntent }) => {
    ...
    if (!isAuthenticated) {
      toast((t) => (
        <span>
          ...
          <a
            href={`/login?mode=login&redirect=${encodeURIComponent(window.location.pathname + window.location.search)}&intent=${encodeURIComponent(intent)}`}
    ...
    if (isAuthenticated && !isVerified) {
      toast((t) => (
        <span>
          ...
          <a
            href={`/verify-pending?redirect=${encodeURIComponent(window.location.pathname + window.location.search)}&intent=${encodeURIComponent(intent)}`}
            className='underline text-yellow-400'
            onClick={() => toast.dismiss(t.id)}
          >Verify now</a>
        </span>
  ...
  return (
    <WrapperTag
      onClick={handleClick}
      className={`relative group ${inline ? 'inline-block' : ''} cursor-not-allowed opacity-60 select-none`}
      aria-disabled='true'
